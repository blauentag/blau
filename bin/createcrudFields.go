...
func main() {
	...
	app.Initialize("src/github.com/blauentag/blau", "webConfig.json")
	settings.Initialize()
	...
	serverSettings.Initialize(os.Getenv("GOPATH")+"/src/github.com/blauentag/blau", "webConfig.json")
...
`, inputFields[i].Name, capCamel, inputFields[i].Name, inputFields[i].Name)
	...
	if hasNameField {
		...
		`)
		utils.ReplaceTokenInFile(br, "-QUERIES-", `"github.com/blauentag/blau/queries"`)
	} else {
		utils.ReplaceTokenInFile(br, "-COPY_NAME-", "")
